  ...
  .factory('checkPwdUtil', function() {
    return {
      getStrength: function (newVal, oldVal) {
        ...
        if (newVal && newVal.length < 6 || newVal && pattern1.test(newVal) && !pattern3.test(newVal) || /^\d{6,}$/.test(newVal) || /^[a-zA-Z]{6,}$/.test(newVal) || /^[a-zA-Z~!@#$%^&*].{6,}$/.test(newVal)) {
          strength = 1;
        } else if (pattern2.test(newVal) && !pattern5.test(newVal) || pattern3.test(newVal) && !pattern5.test(newVal) || pattern4.test(newVal) && !pattern5.test(newVal)) {
          strength = 2;
